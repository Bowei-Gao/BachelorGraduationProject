...
def e_constraint():
    ...
    """
    # add constraint And xi = yj
    for j in range(number_of_customers):
        rows_j = []
        vari_j = []
        coef_j = []
        rhs_j = number_of_requests[j]
        M = sum_number_of_requirements * 100
        for request in requests[j]:
            vari_j.append('x' + str(request - 1))
            coef_j.append(1)
        vari_j.append('y' + str(j))
        coef_j.append(-M)
        rows_j.append([vari_j, coef_j])
        rhs_y1 = rhs_j - M
        ilp_solver.linear_constraints.add(lin_expr=rows_j, senses='G', rhs=[rhs_y1], names=['and_constraint1' + str(j)])
        rhs_y2 = rhs_j - 1
        ilp_solver.linear_constraints.add(lin_expr=rows_j, senses='L', rhs=[rhs_y2], names=['and_constraint2' + str(j)])
    """
    ...
    for i in range(sum_cost + 1):
        ilp_solver.linear_constraints.set_rhs('cost_constraint', i)
        # solve
        ilp_solver.solve()
        # check solver status
        ...
        if 'optimal' in status:
            ...
            elif solutions[j][0] < solution[0]:
                """
                len_solutions = len(solutions)
                for j in range(len_solutions):
                    if
                """
                solutions.append(solution)
